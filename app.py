...
def api_admin_delete_txs(admin_pin: str, student_id: str, tx_ids: list[str]):
    """✅ 관리자 전용: 통장 내역 항목을 완전히 삭제(하드 삭제)
    - 되돌리기(api_admin_rollback_selected)와 달리 원거래 자체를 지워서 화면/합계에서 사라지게 함
    - 그만큼 학생 balance를 되돌려 총액에 반영
    - 적금(가입/만기/해지)·투자·되돌리기 관련 내역은 다른 컬렉션(savings/invest_ledger)과
      연결되어 있어 삭제 대상에서 제외(되돌리기 기능과 동일한 제한)
    - 삭제하는 항목이 '월급 ' 입금이면, 그때 국고에 반영됐던 월급 공제 세입도 함께 환급(지출) 처리
    """
    if not is_admin_pin(admin_pin):
        return {"ok": False, "error": "관리자 PIN이 틀립니다."}
    if not student_id or not tx_ids:
    ...
    tx_docs = []
    for tid in tx_ids:
        snap = db.collection("transactions").document(str(tid)).get()
        if not snap.exists:
            continue
        tx = snap.to_dict() or {}
        if str(tx.get("student_id", "") or "") != str(student_id):
            continue
        tx_docs.append((str(tid), tx))
    ...
    for tid, tx in tx_docs:
        ttype = str(tx.get("type", "") or "")
        memo = str(tx.get("memo", "") or "")
        if ttype == "rollback" or "되돌리기" in memo:
            blocked.append((tid, "되돌리기 기록"))
            continue
        if _is_savings_memo(memo) or ttype in ("maturity",):
            blocked.append((tid, "적금 관련 내역"))
            continue
    ...
    if not valid:
        msg = "선택한 항목이 모두 삭제 불가합니다."
        if blocked:
            msg += " (적금/투자/되돌리기 관련 내역은 삭제할 수 없어요)"
        return {"ok": False, "error": msg}

    # ✅ 월급 공제 세입 환급 계산용: 직업별 공제액 맵(직업/월급 탭의 _calc_net과 동일한 계산식)
    job_to_deduction = {}
    try:
        ...
        for d in db.collection("job_salary").stream():
            jd = d.to_dict() or {}
            jn = str(jd.get("job", "") or "")
            gross = int(jd.get("salary", 0) or 0)
            net = int(_system_calc_net(gross, salary_cfg or {}) or 0)
            if jn and (jn not in job_to_deduction):
                job_to_deduction[jn] = int(max(0, gross - net))
    except Exception:
        job_to_deduction = {}

    student_snap0 = student_ref.get()
    student_name = str((student_snap0.to_dict() or {}).get("name", "") or "") if student_snap0.exists else ""

    now_kst = datetime.now(KST)
    del_tag = f"{now_kst.month:02d}.{now_kst.day:02d}."

    deleted, tre_reversed = 0, 0
    errors = []
    ...
    for tid, tx in valid:
        ...
        ttype = str(tx.get("type", "") or "")
        tx_ref = db.collection("transactions").document(tid)

        try:
            @mongo.transactional
            def _do_one(transaction, _amount=amount, _tx_ref=tx_ref):
                st_snap = student_ref.get(transaction=transaction)
                if not st_snap.exists:
                    raise ValueError("학생 계정을 찾지 못했습니다.")
                bal = int((st_snap.to_dict() or {}).get("balance", 0) or 0)
                new_bal = int(bal - _amount)
                transaction.update(student_ref, {"balance": new_bal})
                transaction.delete(_tx_ref)
                return new_bal

            _do_one(db.transaction())
            deleted += 1
        except Exception as e:
            errors.append(f"[삭제실패] tx={tid} memo={memo} err={e}")
            continue

        # ✅ 월급 입금 삭제라면, 그때 국고로 들어간 공제 세입도 함께 환급(지출)
        is_salary_correction = ("(자동중복정정" in memo) or ("(전체회수" in memo)
        if ttype == "deposit" and amount > 0 and memo.startswith("월급 ") and not is_salary_correction:
            job_name = memo.replace("월급 ", "", 1).strip()
            ded = int(job_to_deduction.get(job_name, 0) or 0)
            if ded > 0:
                tre_res = api_add_treasury_tx(
                    admin_pin=admin_pin,
                    memo=f"월급 공제 세입 환급(항목삭제 {del_tag}) {job_name}" + (f" - {student_name}" if student_name else ""),
                    income=0,
                    expense=ded,
                    actor="admin_delete_tx",
                    recorder_override="관리자",
                )
                if tre_res.get("ok"):
                    tre_reversed += 1
                else:
                    errors.append(f"[국고환급실패] tx={tid} job={job_name} err={tre_res.get('error')}")
    ...
    info_msg = None
    if blocked:
        info_msg = f"삭제 제외 {len(blocked)}건(적금/투자/되돌리기 관련 내역)은 건너뛰었습니다."

    return {"ok": True, "deleted": deleted, "treasury_reversed": tre_reversed, "errors": errors, "message": info_msg}
                ...
                with st.expander(collapsed, expanded=False):
                    ...
                    if not txr.get("ok"):
                        ...
                        if df_tx.empty:
                            ...
                            # 🗑️ 통장내역 삭제(관리자)
                            # -------------------------
                            del_mode_key = f"ind_del_mode_{sid}"
                            if st.button("🗑️ 통장내역 삭제", key=f"ind_del_toggle_{sid}", use_container_width=True):
                                st.session_state[del_mode_key] = not st.session_state.get(del_mode_key, False)

                            if st.session_state.get(del_mode_key, False):
                                st.markdown("#### 🗑️ 삭제할 내역 선택")
                                st.caption(
                                    "체크한 내역을 완전히 삭제합니다(목록/총액에서 즉시 제외). "
                                    "적금·투자·되돌리기 관련 내역은 삭제할 수 없어요. "
                                ...
                                view_df_del = df_tx.copy()
                                view_df_del["가능"] = view_df_del.apply(_can_delete_row, axis=1)
                                deletable_ids = [
                                    str(x) for x in view_df_del.loc[view_df_del["가능"], "tx_id"].tolist()
                                ]

                                all_key = f"ind_del_all_{sid}"

                                def _toggle_all_del(_sid=sid, _ids=list(deletable_ids), _all_key=all_key):
                                    val = bool(st.session_state.get(_all_key, False))
                                    for tid in _ids:
                                        st.session_state[f"ind_del_ck_{_sid}_{tid}"] = val

                                st.checkbox(
                                    "전체 선택",
                                    key=all_key,
                                    on_change=_toggle_all_del,
                                    disabled=(not deletable_ids),
                                )

                                selected_del_ids = []
                                for _, rr in view_df_del.iterrows():
                                    tx_id = str(rr["tx_id"])
                                    can = bool(rr["가능"])
                                    label = f"{rr['created_at_kr']} | {rr['memo']} | +{int(rr['deposit'])} / -{int(rr['withdraw'])}"
                                    ck = st.checkbox(
                                        label,
                                        key=f"ind_del_ck_{sid}_{tx_id}",
                                        disabled=(not can),
                                    )
                                    if ck and can:
                                ...
                                if do_delete_clicked:
                                    if not selected_del_ids:
                                        ...
                                        res_del = api_admin_delete_txs(admin_del_pin, sid, selected_del_ids)
                                        if res_del.get("ok"):
                                            msg = f"삭제 완료: {int(res_del.get('deleted', 0))}건"
                                            if res_del.get("treasury_reversed"):
                                                msg += f" / 국고환급 {int(res_del.get('treasury_reversed', 0))}건"
                                            toast(msg, icon="🗑️")
                                            if res_del.get("message"):
                                                st.info(res_del.get("message"))
                                            if res_del.get("errors"):
                                                with st.expander("일부 실패 상세", expanded=False):
                                                    for e in res_del.get("errors", []):
                                                        st.write(f"- {e}")
